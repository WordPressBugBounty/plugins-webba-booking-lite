...
const placeHolders: Record<"name" | "text" | "content", string | ((editor: any) => string)>[] = [
  ...
  {
    name: "wbk_daily_date_range",
    text: __("Daily date range", "webba-booking-lite"),
    content: "#daily_date_range",
  },
  {
...
const editorConfigs: InitOptions = {
  ...
  external_plugins: {
    ...
    wpeditimage: "/wp-includes/js/tinymce/plugins/wpeditimage/plugin.min.js",
  ...
  return ({ name, label, misc }) => {
    ...
    return (
      <div>
        ...
        {showErrors && <div className="wbk_genericFormField__errorContainer">{firstError}</div>}
      </div>
    );
  };
};

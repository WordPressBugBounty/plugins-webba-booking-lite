...
class WBK_Frontend_Booking
{
    ...
    public function __construct()
    {
        ...
        add_shortcode('webba_email_landing', [
            $this,
            'wbk_email_landing_shortcode',
        ]);
        add_shortcode('webba_multi_service_booking', [
            $this,
            'wbk_shc_multi_service_booking',
        ]);
        // init scripts
        ...
        add_shortcode('webba_user_dashboard', [
            $this,
            'wbk_user_dashboard_shortcode',
        ]);
    }
    ...
    {
        extract(shortcode_atts(['service' => '0'], $attr));
        extract(shortcode_atts(['category' => '0'], $attr));
        extract(shortcode_atts(['category_list' => '0'], $attr));
        if ($service != '0') {
    ...
    public function param_processing()
    {
        ...
        if (isset($_GET['ggeventadd'])) {
            ...
            $appointment_ids = WBK_Model_Utils::get_booking_ids_by_group_token(
                $ggeventadd
            );
            ...
            if ($_GET['pp_aprove'] == 'true') {
                if (isset($_GET['paymentId']) && isset($_GET['PayerID'])) {
                    ...
                    if ($init_result === false) {
                        ...
                        $execResult = $paypal->execute_payment(
                            $paymentId,
                            $PayerID
                        );
                        if ($execResult === false) {
                            ...
                            $pp_redirect_url = trim(
                                WBK_Options_Utils::get_paypal_redirect_url()
                            );
                            if ($pp_redirect_url != '') {
                                if (
                                    filter_var(
        ...
        if(isset($_GET['payment_intent']) && isset($_GET['redirect_status'])) {
            ...
            if($redirect_status == 'succeeded') {
                ...
                if (
                    WBK_Stripe::is_currency_zero_decimal(
                        get_option('wbk_stripe_currency', '')
                    )
                ) {
    ...
    {
        extract(shortcode_atts(['service' => '0'], $attr));
        extract(shortcode_atts(['units' => 'no'], $attr));
        extract(shortcode_atts(['category' => '0'], $attr));
        extract(shortcode_atts(['category_list' => 'no'], $attr));
        extract(shortcode_atts(['location' => '0'], $attr));
        extract(shortcode_atts(['staff' => '0'], $attr));
        extract(shortcode_atts(['compatibility' => 'no'], $attr));
        extract(shortcode_atts(['hide_category' => 'no'], $attr));
        ...
        $cnt = WBK_Renderer::load_template(
    ...
    public function wp_enqueue_scripts()
    {
        ...
        $select_date_placeholder = WBK_Validator::alfa_numeric(
            get_option('wbk_date_input_placeholder', '')
        );

        // Localize the script with new data
        $checkout_label = get_option('wbk_checkout_button_text', '');

        $checkout_label = str_replace(
            '#selected_count',
            '<span class="wbk_multi_selected_count"></span>',
            $checkout_label
        );
        $checkout_label = str_replace(
            '#total_count',
            '<span class="wbk_multi_total_count"></span>',
            $checkout_label
        );
        $checkout_label = str_replace(
            '#low_limit',
            '<span class="wbk_multi_low_limit"></span>',
            $checkout_label
        );
    ...
    {
        if (
            get_option('wbk_initial_dashboard_shortcode_tracked', '') !=
            'true'
        ) {
            WBK_Mixpanel::track_event(
                '1st dashboard shortcode rendering',
                []
            );
            update_option(
                'wbk_initial_dashboard_shortcode_tracked',
                'true'
            );
        }
        return WBK_Renderer::load_template(
            'frontend_v6/user_dashboard',
            [],
            false
        );

...
class WBK_Assets_Manager
{
    ...
    public function __construct($css, $js, $directory = '')
    {
        ...
        add_action(
            'admin_enqueue_scripts',
            [$this, 'admin_enqueue_scripts_textdomain'],
            200
        );
    ...
    public function admin_enqueue_scripts()
    {
        ...
        $translation_array = [
            ...
            'clear' => __('Clear'),
            'close' => __('Close'),
            'duplication_warning' => __(
                'Duplication of bookings is highly discouraged because it can lead to errors in determining free timeslots.',
                'webba-booking-lite'
            ),

            'wbkb_nonce' => wp_create_nonce('wbkb_nonce'),
            'ajaxurl' => admin_url('admin-ajax.php'),
            'empty_table' => esc_html__(
                'No data available',
                'webba-booking-lite'
            ),
            'nofication_icon' =>
        ...
        if (
            isset($_GET['page']) &&
            ($_GET['page'] == 'wbk-calendar' || $_GET['page'] == 'wbk-options')
        ) {
    ...
    public function wp_enqueue_scripts()
    {
        ...
        $has_shortcode = false;
        if (
            $this->has_shortcode('webba_booking') ||
            $this->has_shortcode('webba_email_landing') ||
            $this->has_shortcode('webbabooking') ||
            $this->has_shortcode('webba_multi_service_booking') ||
            $this->has_shortcode('webba_user_dashboard')
        ) {
            $has_shortcode = true;
        }
        ...
        if (
            isset($_GET['action']) &&
            $_GET['action'] == 'oxy_render_oxy-site-navigation'
        ) {
            return;
        }
        if (
            isset($_GET['action']) &&
            $_GET['action'] == 'oxy_render_oxy-shape-divider'
        ) {
            return;
        }
        if (
            isset($_GET['action']) &&
            $_GET['action'] == 'oxy_render_oxy-mini-cart'
        ) {
            return;
        }
        ...
        foreach ($this->js as $item) {
            if ($item[0] == 'frontend6' && $has_shortcode) {
                wp_enqueue_script($item[2], $item[3], $item[4], $item[5], true);
            }
        }
    }

...
class WBK_User_Utils
{
    ...
    {
        global $wpdb;
        $result = get_users(["role__not_in" => ["administrator"]]);
        $result_converted = [];
        foreach ($result as $item) {
            $result_converted[$item->ID] = $item->display_name;
        }
    ...
    public static function check_access_to_service($allow_service_update = false)
    {
        ...
        if ($allow_service_update) {
            $users = $wpdb->get_col(
                "SELECT users FROM " .
                    get_option("wbk_db_prefix", "") .
                    'wbk_services WHERE users_allow_edit="yes"',
            );
        } else {
            $users = $wpdb->get_col(
                "SELECT users FROM " . get_option("wbk_db_prefix", "") . "wbk_services",
            );
        }
     ...
     * @return bool true if has access
     */
    public static function check_access_to_particular_service(
        $service_id,
        $allow_service_update = false
    ) {
        $user_id = get_current_user_id();
        if (current_user_can("manage_options")) {
            return true;
        ...
        if ($allow_service_update) {
            $users = $wpdb->get_col(
                "SELECT users FROM " .
                    get_option("wbk_db_prefix", "") .
                    'wbk_services WHERE users_allow_edit="yes" and id=' .
                    $service_id,
            );
        } else {
            $users = $wpdb->get_col(
    ...
    /**
     * Check if a WordPress user is assigned to any staff member.
     *
     * @param int|string $user_id WordPress user ID.
     * @return bool True if at least one staff member uses this user.
     */
    public static function is_wp_user_used_by_staff_member($user_id)
    {
        ...
        global $wpdb;
        $table = get_option("wbk_db_prefix", "") . "wbk_staff_members";
        $count = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) FROM " . $table . " WHERE wordpress_user = %d",
                (int) $user_id
            )
        );

...
<div class="wbk-outer-container wbk_booking_form_container">
    <div class="wbk-inner-container">
        <img src=<?php echo WP_WEBBA_BOOKING__PLUGIN_URL . '/public/images/loading.svg' ?>
        ...
        <div class="wbk-frontend-row" id="wbk-service-container">
            <div class="wbk-col-12-12">
                ...
                if ($skip_services != '1') {
                    ?>
                            <label
                                class="input-label-wbk wbk-service-category-label <?php echo $item_class; ?>"><?php echo esc_html(get_option('service-label-wbk', 'Select service')); ?></label>
                            <?php
                }
                foreach ($services as $service_id => $service_name) {
                    ...
                    if ($skip_services == '1') {
                        ?>
                                    <input type="checkbox" value="<?php echo esc_html($service_id) ?>"
                                        class="wbk-checkbox wbk-service-checkbox"
                                        id="wbk-service_chk_<?php echo esc_html($service_id) ?>" checked />
                                    <?php
                    } else {
                        ?>
                                    <input type="checkbox" value="<?php echo esc_html($service_id) ?>"
                                        class="wbk-checkbox wbk-service-checkbox"
                                        id="wbk-service_chk_<?php echo esc_html($service_id) ?>" />
                                    <?php
                    }
                    ?>
                            <label for="wbk-service_chk_<?php echo esc_html($service_id) ?>"
                                class="wbk_service_chk_label_<?php echo esc_html($service_id) ?> wbk-checkbox-label wbk_service_chk_label <?php echo ' ' . esc_html($item_class) . ' '; ?>  wbk-dayofweek-label"><?php echo esc_html($service_name); ?></label>
                            <div
                                class="wbk_chk_clear_<?php echo esc_html($service_id) ?> wbk-clear <?php echo esc_html($item_class) ?>">
                            </div>
                            <?php
                }
                if ($skip_services == '1') {
        ...
        <div class="wbk-frontend-row">
            <input type="button" disabled="disabled"
                class="<?php echo $start_btn_class; ?> wbk-button wbk-width-100 wbk-mt-10-mb-10"
                id="wbk-confirm-services" value="<?php echo __('Start booking', 'webba-booking-lite'); ?>">
        </div>

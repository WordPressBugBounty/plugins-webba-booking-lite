...
<div class="wbk-col-12-12">
        ...
        if ($sp->is_working_day($i, $service_id) || $sp->is_unlockced_has_dow($i, $service_id)) {
            ...
            <div class="<?php echo esc_attr($row_class); ?> ">
                    ...
                    foreach ($intervals as $interval) {
                        ...
                        for ($time = $start; $time <= $end - $service->get_duration() * 60; $time += $hours_step) {
                            ...
                            <option value="<?php echo esc_attr($time); ?>">
                                <?php echo esc_html(__('from', 'webba-booking-lite')) . ' ' . wp_date($time_format, $time, new DateTimeZone('UTC')); ?>
                            </option>
                            <?php
                        }
                    }
                ...
                <div class="wbk-col-3-12 wbk-table-cell">
                    <input type="checkbox" value="<?php echo WBK_Validator::alfa_numeric($day_name); ?>" class="wbk-checkbox"
                        id="wbk-day_<?php echo WBK_Validator::alfa_numeric($day_name); ?>" checked="checked" />
                    <label for="wbk-day_<?php echo WBK_Validator::alfa_numeric($day_name); ?>"
                        class="wbk-checkbox-label"><?php echo esc_html($day_name_translated); ?></label>
                </div>
    ...
    ?>
    <div class="wbk-clear"></div>
    <input type="button" class="wbk-button" id="wbk-search_time_btn"
        value="<?php echo esc_html(__('Search time slots', 'webba-booking-lite')); ?>">
    <?php
    ?>